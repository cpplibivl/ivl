...
public:
    ...
    float_point(const point<float>& p) : base_class(p) { }
    
    // is this relaxing restriction?
    template<class T, class D>
    float_point(const array<T, D>& p) : base_class(p[0], p[1]) { }
...
public:
    ...
    double_point(const point<double>& p) : base_class(p) { }
    
    // is this relaxing restriction?
    template<class T, class D>
    double_point(const array<T, D>& p) : base_class(p[0], p[1]) { }
