...
/**@}*/
	...
	{
		//this only handles swap(this_type)
		//add an implementation for different sizes of N. Note: tricky.

		//swap length
		size_t old_len = this->len;
		this->len = v.len;

		//both small: swap contents
		if(this->len <= N && v.len <= N) {
			int ml = std::max(this->len, v.len);
			for(size_t i = 0; i < ml; i++)
				std::swap(this->base_ptr[i], v.base_ptr[i]);
			return;
		}		

		//swap pointers
		T* old_ptr = this->base_ptr;
		this->base_ptr = v.base_ptr;

		this_type* small = 0;
		if(this->len <= N) small = this;
		else if(v.len <= N) small = *v;

		//one out of two is small
		if(small) {
			for(size_t i = 0; i < small->len; i++)
				small->dt[i] = small->base_ptr[i];
			small->base_ptr = small->dt;
		}
	}
