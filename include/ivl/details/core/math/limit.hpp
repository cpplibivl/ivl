...
//! infinity of double
template<typename T> inline T infty() { return std::numeric_limits<T>::infinity(); }

//! largest positive (but not INF)
...
//! epsilon (smallest) numeric limit. different from epsilon_constant
template<typename T> inline T eps()   
{ 
...
{
	template<class T>
	T operator()(const T&) const { return ivl::infty <T> (); }
};
...
{
	template<class T>
	T operator()(const T&) const { return ivl::eps <T> (); }
};
